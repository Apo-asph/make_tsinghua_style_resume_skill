...
- 子版必须遵守模板规定的无框表格结构；未经用户明确要求，不得改写页眉、分节、经历条目、技能表等整体排版结构，也不得改成带边框或其他表格样式。
- 允许调整的版式参数仅限：中英文本切换、模板既有的字号与行距/密度参数，以及无框表格列宽；其中列宽只用于修复日期、地点、邮箱、链接、学校/机构名等原本预期为单行的短文本换行问题。
...
- 若页面底部留白明显大于顶部，即使页数仍为 1 页，也视为版面未完成；需要继续调整版面直到符合提交要求，不能直接按可提交处理。
- 以下元素默认必须保持单行：姓名、标题、联系方式整行、节标题、日期、地点、技能表左列标签，以及邮箱、链接等短字段；若这些字段换行，优先精简文本或微调允许的参数，不得直接改模板结构。
...
- 若姓名、标题、联系方式、节标题、日期、地点、技能标签、邮箱或链接等原本预期为单行的短文本发生换行，优先缩写或精简文本，其次微调无框表格列宽或中英表达，不要直接改模板结构。
...
- 若出现以下问题，先汇报，并说明需要继续调整版面直到符合提交要求；若用户未预先授权连续微调，再等待用户是否批准继续微调：
  - 页数超过目标；
  ...
  - 文本、链接或项目符号溢出；
  - 证件照位置生硬或与页眉不齐。
- 其中“正文整体偏短，未自然铺满页面”与“底部留白显著大于顶部留白”都不能按可提交处理；需要继续调整版面直到符合提交要求。
...
- 获得用户批准后，按以下顺序修正版式问题：
  ...
  4. 如果原本预期为单行的短文本字段发生换行，优先精简文本或切换更合适的中英表达，再在必要时微调无框表格列宽；
  5. 仅在仍然需要时再调模板中的全局间距常量。
- 除非用户明确要求，不做大规模视觉重设计。
